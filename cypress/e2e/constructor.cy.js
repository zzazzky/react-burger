...
describe('order modal work correctly', function () {
  ...
  it('should open order modal', function () {
    cy.get('#react-modals').children().should('not.exist');
    cy.get('[data-cy=order-button-container]')
      .contains('Оформить заказ')
      .click();
    cy.get('form')
      .get('input[name="email"]')
      .type('v555@m.ru')
      .should('have.value', 'v555@m.ru');
    cy.get('form')
    ...
    cy.get('[data-cy=order-button-container]')
      .contains('Оформить заказ')
      .click();
    cy.get('#react-modals').children().should('exist');
  });
